...
entropy_rf = joblib.load('entropy_model.joblib')
  ...
  def _handle_openflow_PacketIn (self, event):  
    ...
    if isinstance(packet.next, ipv4):
      ...
      #If flag trigger is True when anomaly alarm is raised by entropy engine and the classifier is started
        ...
        loaded_rf = joblib.load('trained_model.joblib')
        flag_classifier_output = loaded_rf.predict(dummy_list)
...
def handle_flow_stats (event):
  ...
  cur_stat_y_predicted = entropy_rf.predict(cur_stat_y)
